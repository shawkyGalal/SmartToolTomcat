  ...
  if ( id != null)  
  {
	...
	{	Misc.initializeMainSystemTrees(request, response, session, application , out);
      ...
      String pathLinks = "";
		...
		catch( Exception e ) {throw new Exception ("System/Module You are Trying to Reach (Module ID : "+id+") is not found in the Main System Tree." 
...
<table width="100%" border="0" align="left">
	...
	<tr> 
		...
		<td width="40%">
		<div align="center">
		<a href = "queryResultNonTabbed.jsp?<%=request.getQueryString()%>" title="nonTapVersion"> <img src="<%=appURL%>/images/listIcon.jpg" width="30"></a>
		 <a href = "index.jsp?reportTopOpen=<%=id%>" target = top title="الصفحة الرئيسية"><img src="<%=appURL%>/images/homeIcon.jpg" width="30"></a>
		   ...
		   %> 
		 <a href = "?<%=refreshLink%>" title = "Reload Data إعادة التحميل"  ><img src="<%=appURL%>/images/toolbar_icon_reload_active.gif" width="30"></img></a>
		 <a href = "?<%=printableLink%>" title = "نسخة قابلة للطباعة Printable Version" target = "Printable"><img src="<%=appURL%>/images/cleardot.jpg" width="30" ></a>
		 <a href = "queryResultWithEditableRS.jsp?<%=request.getQueryString() %>" title="Click To Update إضغط للتعديل " > <img src="<%=appURL%>/images/editList.jpg" width="30" ></a>
		 <img  title="تغيير إتجاه الجدول" src="<%=appURL%>/images/direction.jpg" 
		       onclick="
		...
		<td width="20%"><font color="#FF0000" size="2"><em> (<%=Misc.getConnectionUserName(con)%>@<%=(selectedConnParms != null )? selectedConnParms.name : ""%>)</em></font></td>
	</tr>
</table>
<%} %>
</div>
