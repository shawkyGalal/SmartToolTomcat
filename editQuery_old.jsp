...
<table width="92%" border="1">
	...
	<tr>
		...
		<td><textarea name="QUERY_BODY" cols="100" rows="20" wrap="OFF"><%=QUERY_BODY%></textarea>
			<a href = "javaScript:loadDetails('refrencedQueryList.jsp?queryId=<%=request.getParameter("id")%>', 'sysMessage')" >Display Referenced Queries </a>
			<!--  onmouseover = "setTimeout( function () { loadDetails('refrencedQueryList.jsp?queryId=<%=request.getParameter("id")%>', 'sysMessage') }, 8000) ; " -->
		<script>
		  $(function() {
		    $( "#sysMessage" ).dialog(
