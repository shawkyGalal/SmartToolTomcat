...
%>
<p align="center"><strong><font size="5"><%=tableObjectName%></font></strong>
</p>
...
<% }
  ...
  counter = 0 ; 
  %>
  <form name="<%=tableName+"_InsertForm"%>" id="<%=tableName+"_InsertForm"%>" action="TableInsertForm2.jsp" method="post">
  <table width="100%" border="0" dir="rtl" align = "center">
  	<tr>
  		<td>
  ...
  { counter++;
   ...
   if ( ( columnValue == null || columnValue.equals("")) && defaultValue != null )
   {
	...
	%>
		...
		<tr>
			<td width="5%"></td>
			<td title="<%=comments%>" align="right" dir = "rtl" ><strong><%=((html_type == 6)? "": displayName_)%>. <%=((html_type == 6)? "": displayName )%></strong></td>
			<td></td>
			<td></td>
		</tr>
		<tr>
	...
	{	String operationMode = (html_type == 14 )? LookupTreeV10.MULTI_SELECT_OPERATION_MODE :LookupTreeV10.SINGLE_SELECT_OPERATION_MODE ;
		...
		%> <textarea data-validation-engine ="<%=jQueryValidatorClass %>" readonly <%=htmlControlDisabled? "disabled=disabled" : "" %> id=<%=columnName%> name = <%=columnName%>  <%=attribute%> onchange="  updaetHref_<%=columnName%>( this ) ;  " ><%=columnValue%></textarea>
			<a target = "tree Selection"  id="<%=columnName%>_TreeLink"
      					...
      					href = "javascript:window.open('selectionTree.jsp?refreshAll=xx&_operationMode=<%=operationMode%>&_querySouce=<%=treeQuerySource%>&_selectedIDs=<%=columnValue%>&treeIdInSession=<%=owner %>/<%=tableName%>/<%=columnName%>&_fillObject=<%=columnName%>' , 'Select From Tree' , 'width=400, height=600' ) " ><img width="25" height = "25"  src="images/treeIcon.jpg"> </a>
		<div id="<%=columnName%>_label"></div>
		
	 	<script type="text/javascript">
	 			function updaetHref_<%=columnName%>(m_object)
	 			{
	 				var newhref = "javascript:window.open('selectionTree.jsp?_operationMode=<%=operationMode%>&_querySouce=<%=treeQuerySource%>&_selectedIDs=" + m_object.value + "&treeIdInSession=<%=owner %>/<%=tableName%>/<%=columnName%>&_fillObject=<%=columnName%>' , 'Select From Tree' , 'width=450, height=600' )" ; 
...
<div align="center">
	...
	<% } %>
	<a href = 'javascript: closeWindow(); ' > إغلاق - Close </a>	
	<input class="submit" type="submit" name="Save" value="Save حفظ" onclick ="return formValidate(); "></p>
	<%  
	   if (smartToolAdmin) 
	   { %>
