...
<BODY
	onLoad='initTable("table0");initTable("table1");initTable("table2");
		initTable("table3");initTable("table4");initTable("table5");
    initTable("table6");initTable("table7");initTable("table8");initTable("table9");'>
<body>
<%
...
<table ID="table<%=j%>" border=1 cellspacing="0" cellpadding="0">
	<TBODY>
		<tr>
			<%
        ...
        { 
          ...
          %>
			<th bgcolor="#99CC66"><%=Support.Misc.repalceAll(columnNames[count],"_"," ")%>
			</th>
			<%
         }
        %>
        ...
        { rownum++;
          ...
          { 
			...
			<td><%=columnValue%> <!--
         <input name="Det" type="button" onClick=" longTextViewer= window.open(); 
...
<%
    ...
    System.out.println("User : " + Misc.getConnectionUserName(con) + " Executed Query No " + request.getParameter("id") + " Title: " + titles[0]);
    stmt.close();
    if( AutoLog)
    {
    con.close();
